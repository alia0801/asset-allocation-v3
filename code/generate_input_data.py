    ...
    for i in range(number):
        ...
        for j in range(len(group)):
            ...
            if j!=len(group)-1:
                ...
                sql += ('`'+etf+'`')
        # SELECT * from (SELECT date,SPY FROM close WHERE date < '2020-03-01' order by date DESC limit 21) AS C order by C.date
        sql_close  = "SELECT * from (" + sql + (" from `close`  where date < '"+str(date1)+"' order by date DESC limit 756) AS C order by C.date")
        sql_close2 = "SELECT * from (" + sql + (" from `close`  where date < '"+str(date1)+"' order by date DESC limit 777) AS C order by C.date")
        sql_volume = "SELECT * from (" + sql + (" from `volume` where date < '"+str(date1)+"' order by date DESC limit 756) AS C order by C.date")
        # print(sql_close)
        # print(sql_volume)
        cursor.execute(sql_close)
        result_close = cursor.fetchall()
