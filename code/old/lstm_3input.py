...
def ecm(input_p):
    print(input_p)
    i_p = []
    for i in range(input_p.shape[0]):
      i_p.append(input_p[i][0])
    input_point = np.array(i_p)
    print(input_point)
...
def LSTM_a_batch( iii, batch_state, batch_output, outputs  ):#iii.shape=(21,3) iii[j].shape=(3,)
...
# print(len(y))
# print(len(y_model_cal))
# for i in range(len(y_model_cal)):
#   print(y[i],y_model_cal[i])
mse = np.sqrt( ( ( np.array(y_model_cal) - np.array(y_cal_mse) ) ** 2).mean() )
print('total mse =', mse)
